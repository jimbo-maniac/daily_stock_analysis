...
class BacktestRunRequest(BaseModel):
    code: Optional[str] = Field(None, description="onlybacktestspecifiedstock")
    force: bool = Field(False, description="mandatoryre-calculating")
    eval_window_days: Optional[int] = Field(None, ge=1, le=120, description="evaluationwindow（trading daycount）")
    min_age_days: Optional[int] = Field(None, ge=0, le=365, description="analyzingrecordmostsmalldaysage（0=notlimit）")
    limit: int = Field(200, ge=1, le=2000, description="at mostprocessinganalyzingrecordcount")


class BacktestRunResponse(BaseModel):
    processed: int = Field(..., description="candidaterecordcount")
    saved: int = Field(..., description="writingbacktest resultcount")
    completed: int = Field(..., description="completedbacktestcount")
    insufficient: int = Field(..., description="datainsufficientcount")
