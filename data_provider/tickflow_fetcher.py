...
_CN_MAIN_INDEX_QUOTES = (
    ("000001.SH", "000001", "Shanghai Compositeindex"),
    ("399001.SZ", "399001", "Shenzhen Component Index"),
    ("399006.SZ", "399006", "ChiNextrefers to"),
    ("000688.SH", "000688", "STAR50"),
    ("000016.SH", "000016", "Shanghai Composite50"),
    ("000300.SH", "000300", "Shanghai-Shenzhen300"),
)
...
class TickFlowFetcher(BaseFetcher):
    ...
    def close(self) -> None:
        ...
        if client is not None:
            try:
                ...
                logger.debug("[TickFlowFetcher] closeclientfailed: %s", exc)
    ...
    def get_main_indices(self, region: str = "cn") -> Optional[List[Dict[str, Any]]]:
        ...
        if not quotes:
            logger.warning("[TickFlowFetcher] indexquote/market datais empty")
            return None
        ...
        if len(results) != len(_CN_MAIN_INDEX_QUOTES):
            logger.warning(
                "[TickFlowFetcher] indexquote/market dataincomplete: %s/%s",
                len(results),
    ...
    def get_market_stats(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if self._is_universe_permission_error(exc):
                ...
                self._universe_query_checked_at = now
                logger.info(
                    "[TickFlowFetcher] currentpackagenot supportedmarkpoolquerying，market statisticsrollbacktocurrenthasdatasource"
                )
                return None
            raise
        if not quotes:
            logger.warning("[TickFlowFetcher] market statisticsquote/market datais empty")
            return None
        ...
        for quote in quotes:
            ...
            if not name:
                logger.debug("[TickFlowFetcher] lackstockname，bynon- ST processing: %s", symbol)
        ...
        if valid_rows == 0:
            logger.warning("[TickFlowFetcher] market statisticsnothitvalid A stocksquote/market data")
            return None
