...
_NAME_ALIASES = frozenset({"name", "stockname", "name", "stock_name"})
...
def _should_use_single_column_fast_path(lines: List[str]) -> bool:
    ...
    for ln in lines:
        ...
        if len(parts) >= 2 and is_code_like(parts[0]):
            # Example: "600519 Kweichow Moutai" / "HK00700 Tencent Holdings"
...
def _parse_dataframe(df: pd.DataFrame) -> List[Tuple[Optional[str], Optional[str], str]]:
    ...
    for _, row in df.iterrows():
        ...
        if code_val:
            ...
            # (do not overwrite valid name with dirty code_val, e.g. INVALID,Kweichow Moutai)
...
def parse_import_from_bytes(data: bytes, filename: Optional[str] = None) -> List[Tuple[Optional[str], Optional[str], str]]:
    ...
    if len(data) > MAX_FILE_BYTES:
        raise ValueError(f"fileexceed {MAX_FILE_BYTES // (1024 * 1024)}MB constraint")

    ext = ""
    if filename:
        ext = "." + filename.rsplit(".", 1)[-1].lower() if "." in filename else ""
    logger.debug(f"[ImportParser] startingparsingfile: filename={filename or '-'}, ext={ext or '-'}, bytes={len(data)}")
    ...
    if ext == ".xlsx" or looks_like_zip:
        try:
            ...
            if looks_like_zip:
                hint = (
                    "pleaseconfirm：(1) fileas .xlsx format；(2) worktablenotis empty；(3) filenotdamaged。"
                    "ifas .xls format，pleasesave asas .xlsx afterretry。"
                )
            ...
            logger.warning(f"extensionnameas .xlsx butnotparsingas Excel，willrollbacktextparsing: {e}")

    # .xls not supported
    if ext == ".xls":
        raise ValueError("only support .xlsx format，pleasewill .xls save asas .xlsx afterretry")
    ...
    for encoding in ("utf-8", "gbk"):
        ...
        raise ValueError("unable toidentifyfileencoding，please use UTF-8 or GBK")
    ...
    try:
        ...
        raise ValueError(
            f"CSV parse failed：pleasecheckseparatesymbolwhetherconsistent、columncountwhethermatch。"
            f"normallyseereason：quotesnotclose、certainrowcolumncountwithotherrowinconsistent。rawerror: {e}"
        ) from e
    except Exception:
        pass
...
def parse_import_from_text(text: str) -> List[Tuple[Optional[str], Optional[str], str]]:
    ...
    if len(text.encode("utf-8")) > MAX_TEXT_BYTES:
        raise ValueError(f"textexceed {MAX_TEXT_BYTES // 1024}KB constraint")

    logger.debug(f"[ImportParser] startingparsingpastetext: bytes={len(text.encode('utf-8'))}")
