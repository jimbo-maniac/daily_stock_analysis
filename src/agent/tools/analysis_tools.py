...
def _handle_calculate_ma(stock_code: str, periods: Optional[str] = None, days: int = 120) -> dict:
    ...
    result["ma_alignment"] = (
        "long positionarrange" if above_count == len(ma_values)
        else "short positionarrange" if above_count == 0
        else f"mixed({above_count}/{len(ma_values)}itemsmoving averageabove/uppermethod)"
    )
    return result
...
calculate_ma_tool = ToolDefinition(
    ...
    description="Calculate moving averages (MA5/10/20/30/60/120/250 or custom periods) "
                ...
                "is above each MA. Also returns overall MA alignment (long position/short position/mixed).",
    parameters=[
        ToolParameter(
            name="stock_code",
            type="string",
...
def _handle_get_volume_analysis(stock_code: str, days: int = 30) -> dict:
    ...
    if len(volume) >= 10:
        ...
        vol_trend = "volume increase" if vol_trend_pct > 20 else "volume contraction" if vol_trend_pct < -20 else "volumestable"
    else:
        vol_trend_pct = 0
        vol_trend = "datainsufficient"
    ...
    if avg_up_vol > avg_down_vol * 1.3:
        pattern = "volume-pricegood coordinationgood（risingvolume increase、fallingvolume contraction）"
    elif avg_down_vol > avg_up_vol * 1.3:
        pattern = "volume-pricedivergence（fallingvolume increase、risingvolume contraction，biasedempty）"
    elif vol_ratio_5d and vol_ratio_5d > 1.5:
        pattern = "recentclearly in periodvolume increase"
    elif vol_ratio_5d and vol_ratio_5d < 0.6:
        pattern = "recentclearly in periodvolume contraction"
    else:
        pattern = "volume-priceclosesystemneutral"
...
get_volume_analysis_tool = ToolDefinition(
    ...
    description="Analyse volume-price relationship for a stock. Returns volume ratios, "
                ...
                "and pattern interpretation (volume-pricecoordination/divergence). Useful for confirming trend "
...
def _handle_analyze_pattern(stock_code: str, days: int = 60) -> dict:
    ...
    for i in range(max(0, n - 3), n):
        ...
        if bd < avg_body * 0.1 and (us + ls) > bd * 3:
            patterns_detected.append({
                "pattern": "tencharacterstar (Doji)", "type": "reversal_signal",
                "day_offset": -(n - 1 - i),
                "strength": "weak", "desc": "multipleemptybalance，possiblychangeplatesignal"
            })

        # Hammer / Hanging Man
        if ls > body(i) * 2 and us < body(i) * 0.5:
            label = "hammersub-line (Hammer)" if i == 0 or c[i] >= c[i - 1] else "hangingline (Hanging Man)"
            patterns_detected.append({
                "pattern": label, "type": "reversal_signal",
                "day_offset": -(n - 1 - i),
                "strength": "in", "desc": "belowshadowlinelong，latentinsupport/reverse"
            })

        # Shooting Star / Inverted Hammer
        if us > body(i) * 2 and ls < body(i) * 0.5:
            label = "streamstarline (Shooting Star)" if is_bearish(i) else "inverted hammersub-"
            patterns_detected.append({
                "pattern": label, "type": "bearish_signal",
                "day_offset": -(n - 1 - i),
                "strength": "in", "desc": "upper shadowlinelong，latentinstress/reverse"
            })

        # Big bullish / bearish candle
        if bd > avg_body * 2.5:
            label = "largebullishline" if is_bullish(i) else "largebearishline"
            t = "bullish" if is_bullish(i) else "bearish"
            patterns_detected.append({
                "pattern": label, "type": t,
                "day_offset": -(n - 1 - i),
                "strength": "strong", "desc": "bodylarge，directioncleardetermine"
            })
    ...
    if n >= 3:
        ...
        if (is_bearish(i - 2) and body(i - 2) > avg_body * 1.5
            ...
            patterns_detected.append({
                "pattern": "morning star (Morning Star)", "type": "bullish_reversal",
                "day_offset": -2, "strength": "strong", "desc": "threerootcandlestickbottomreversepattern"
            })

        # Evening Star (evening star) — top reversal
        if (is_bullish(i - 2) and body(i - 2) > avg_body * 1.5
                and body(i - 1) < avg_body * 0.4
                and is_bearish(i) and body(i) > avg_body * 1.5
                and c[i] < (o[i - 2] + c[i - 2]) / 2):
            patterns_detected.append({
                "pattern": "evening star (Evening Star)", "type": "bearish_reversal",
                "day_offset": -2, "strength": "strong", "desc": "threerootcandlesticktopreversepattern"
            })

        # Engulfing (engulfpattern)
        if (is_bullish(i) and is_bearish(i - 1)
                and o[i] < c[i - 1] and c[i] > o[i - 1]):
            patterns_detected.append({
                "pattern": "bullishengulf (Bullish Engulfing)", "type": "bullish_reversal",
                "day_offset": -1, "strength": "strong", "desc": "bullishlinecompletelyoverridebeforeonebearishline"
            })
        elif (is_bearish(i) and is_bullish(i - 1)
              and o[i] > c[i - 1] and c[i] < o[i - 1]):
            patterns_detected.append({
                "pattern": "bearishengulf (Bearish Engulfing)", "type": "bearish_reversal",
                "day_offset": -1, "strength": "strong", "desc": "bearishlinecompletelyoverridebeforeonebullishline"
            })

    # --- Chart patterns over the window ---
    # Double bottom detection (simplified version: twocountmutualrecentlowpoint + inbetweenhighpoint)
    ...
    if len(recent_lows_idx) >= 2:
        ...
        if lo2 - lo1 >= 5 and abs(l[lo1] - l[lo2]) / max(l[lo1], l[lo2]) < 0.03:
            ...
            if mid_high > l[lo1] * 1.03:
                patterns_detected.append({
                    "pattern": "double bottom (Double Bottom)", "type": "bullish_reversal",
                    "day_offset": -(n - 1 - lo2),
                    "strength": "strong", "desc": "twocountmutualrecentlowpoint，Wtypebottompattern"
                })
    ...
    if n >= 21:
        ...
        if c[-1] > high_20d and (v is None or v[-1] > sum(v[n - 6:n - 1]) / 5 * 1.5):
            patterns_detected.append({
                "pattern": "volume increasebreakout20dayhighpoint", "type": "bullish_breakout",
                "day_offset": 0, "strength": "strong", "desc": "closebreakoutrecent20dayhighest，volumecoordination"
            })
    ...
    if n >= 10:
        ...
        if box_range_pct < 8:
            patterns_detected.append({
                "pattern": "box patternoscillation", "type": "consolidation",
                "day_offset": 0, "strength": "in",
                "desc": f"recent10dayamplitude {box_range_pct:.1f}%，priceinintervalinoscillation"
            })
    ...
    return {
        ...
        "summary": (
            "notsendcurrentobviouspattern" if not unique_patterns
