...
def _validate_password(pwd: str) -> Optional[str]:
    ...
    if not pwd or not pwd.strip():
        return "passwordcannotis empty"
    if len(pwd) < MIN_PASSWORD_LEN:
        return f"passwordat least {MIN_PASSWORD_LEN} digit"
    return None
...
def change_password(current: str, new: str) -> Optional[str]:
    ...
    """
    if not is_auth_enabled():
        return "authenticationfeature notenabled"
    if not is_password_set():
        return "stillnotsettingspassword"

    if not current or not current.strip():
        return "pleaseinputcurrentpassword"
