...
class TestTickFlowFetcher(unittest.TestCase):
    def test_get_main_indices_maps_cn_quotes(self):
        ...
        self.assertEqual(data[0]["name"], "Shanghai Compositeindex")
    ...
    def test_get_market_stats_calculates_a_share_rules(self):
        ...
        fetcher._client = _FakeClient(
            universe_data=[
                _quote("600000.SH", last_price=11.0, prev_close=10.0, amount=1e8, name="SPDB"),
                _quote("300750.SZ", last_price=12.0, prev_close=10.0, amount=1e8, name="CATL"),
                _quote("688001.SH", last_price=8.0, prev_close=10.0, amount=1e8, name="STARtesting"),
                _quote("920001.BJ", last_price=13.0, prev_close=10.0, amount=1e8, name="northexchangetesting"),
                _quote("600001.SH", last_price=10.5, prev_close=10.0, amount=1e8, name="*STExample"),
                _quote("600002.SH", last_price=10.0, prev_close=10.0, amount=1e8, name="flatExample"),
                _quote("600003.SH", last_price=11.0, prev_close=10.0, amount=0.0, name="zerotrading amount"),
                _quote("600004.SH", last_price=11.0, prev_close=None, amount=1e8, name="missingyesterday close"),
            ]
        )
    ...
    def test_get_market_stats_returns_none_when_universe_query_not_supported(self):
        fetcher = TickFlowFetcher(api_key="sk-test")
        fetcher._client = _FakeClient(
            universe_data=RuntimeError("currentpackagenot supportedmarkpoolquerying，pleaseriseleveloruse symbols parameter")
        )
