...
class PortfolioPr2TestCase(unittest.TestCase):
    ...
    def _csv_bytes(with_trade_uid: bool = True) -> bytes:
        if with_trade_uid:
            csv_text = (
                "trade date,security code,buy/sellflag,trade volume,average trade price,trade number,commission fee,stamp duty\n"
                "2026-01-02,600519,buy,10,100,HT-001,1,0\n"
            )
        else:
            csv_text = (
                "trade date,security code,buy/sellflag,trade volume,average trade price,commission fee,stamp duty\n"
    ...
    def test_import_side_parser_avoids_false_sell_match(self) -> None:
        csv_text = (
            "trade date,security code,buy/sellflag,trade volume,average trade price,trade number\n"
    ...
    @patch.object(PortfolioRiskService, "_fetch_belong_boards", return_value=[{"name": "baijiu", "type": "industry"}])
