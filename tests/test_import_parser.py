...
class TestParseImportFromBytesCsv:
    def test_parses_csv_with_header(self):
        data = "code,name\n600519,Kweichow Moutai\n00700,Tencent Holdings".encode("utf-8")
        result = parse_import_from_bytes(data, "a.csv")
        assert len(result) == 2
        assert result[0] == ("600519", "Kweichow Moutai", "medium")
        assert result[1] == ("00700", "Tencent Holdings", "medium")

    def test_parses_csv_chinese_column_names(self):
        data = "stock code,stockname\n600519,Kweichow Moutai".encode("utf-8")
        result = parse_import_from_bytes(data, "a.csv")
        assert result[0] == ("600519", "Kweichow Moutai", "medium")

    def test_parses_csv_no_header(self):
        # Use 300750 instead of 00700 to avoid pandas stripping leading zeros
        data = "600519,Kweichow Moutai\n300750,CATL".encode("utf-8")
        result = parse_import_from_bytes(data, "a.csv")
        assert len(result) == 2
        assert result[0] == ("600519", "Kweichow Moutai", "medium")
        assert result[1] == ("300750", "CATL", "medium")
    ...
    def test_resolves_name_when_code_empty(self, mock_resolve):
        ...
        data = "code,name\n,Kweichow Moutai".encode("utf-8")
        result = parse_import_from_bytes(data, "a.csv")
        assert result[0] == ("600519", "Kweichow Moutai", "medium")
        mock_resolve.assert_called_with("Kweichow Moutai")

    @patch("src.services.import_parser.resolve_name_to_code")
    def test_returns_none_code_when_resolution_fails(self, mock_resolve):
        mock_resolve.return_value = None
        data = "code,name\n,not foundstock".encode("utf-8")
        result = parse_import_from_bytes(data, "a.csv")
        assert result[0] == (None, "not foundstock", "medium")
...
class TestParseImportFromBytesExcel:
    def test_parses_xlsx(self):
        ...
        ws = wb.active
        ws.append(["code", "name"])
        ws.append(["600519", "Kweichow Moutai"])
        ws.append(["300750", "CATL"])
        buf = io.BytesIO()
        wb.save(buf)
        buf.seek(0)
        data = buf.read()
    ...
    def test_parses_xlsx_without_header(self):
        ...
        ws = wb.active
        ws.append(["600519", "Kweichow Moutai"])
        ws.append(["00700", "Tencent Holdings"])
        buf = io.BytesIO()
        wb.save(buf)
        buf.seek(0)
        data = buf.read()
    ...
    def test_rejects_xls(self):
        data = b"dummy"
        with pytest.raises(ValueError, match="only support .xlsx"):
    ...
    def test_excel_error_includes_actionable_hint(self):
        ...
        assert "pleaseconfirm" in msg or ".xlsx" in msg
...
class TestParseImportLimits:
    ...
    def test_csv_parser_error_raises_helpful_message(self):
        ...
        assert "separatesymbol" in msg or "quotes" in msg

    def test_accepts_gbk_encoded_csv(self):
        # Build CSV with Chinese in GBK encoding
        data = ("code,name\n600519," + "Kweichow Moutai").encode("gbk")
        result = parse_import_from_bytes(data, "a.csv")
        assert len(result) == 1
        assert result[0][0] == "600519"
        assert result[0][1] == "Kweichow Moutai"
...
class TestParseImportFromText:
    ...
    def test_preserves_name_when_code_is_dirty(self):
        data = "code,name\nINVALID,Kweichow Moutai".encode("utf-8")
        result = parse_import_from_bytes(data, "a.csv")
        assert len(result) == 1
        assert result[0] == ("600519", "Kweichow Moutai", "medium")
